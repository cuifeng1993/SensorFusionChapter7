...
ErrorStateKalmanFilter::ErrorStateKalmanFilter(const YAML::Node& node) {
    ...
    COV.PROCESS.B_GYRO = node["covariance"]["process"]["b_gyro"].as<double>();
    COV.PROCESS.B_ACCEL = node["covariance"]["process"]["b_accel"].as<double>();
    // d. measurement noise:
    ...
    Q_.block<3, 3>(6, 6) = COV.PROCESS.B_GYRO*Eigen::Matrix3d::Identity();
    Q_.block<3, 3>(9, 9) = COV.PROCESS.B_ACCEL*Eigen::Matrix3d::Identity();
...
void ErrorStateKalmanFilter::Init(
    ...
    Eigen::Vector3d linear_angular_init(
    ...
    linear_angular_init = (linear_angular_init - gyro_bias_);
    // init process equation, in case of direct correct step:
    UpdateProcessEquation(linear_acc_init,linear_angular_init);
...
bool ErrorStateKalmanFilter::Update(const IMUData &imu_data) {
    ...
    if (time_ < imu_data.time) {
        ...
        Eigen::Vector3d linear_angular_mid;
        imu_data_buff_.push_back(imu_data);
        UpdateOdomEstimation(linear_acc_mid,linear_angular_mid);
        imu_data_buff_.pop_front();

        // update error estimation:
        double T = imu_data.time - time_;
        UpdateErrorEstimation(T, linear_acc_mid,linear_angular_mid);
...
bool ErrorStateKalmanFilter::GetAngularDelta(
    ...
    linear_angular_mid = 0.5*(angular_vel_curr + angular_vel_prev);
    angular_delta = delta_t*linear_angular_mid;
 ...
 * @return void
 */
void ErrorStateKalmanFilter::UpdateOdomEstimation(Eigen::Vector3d &linear_acc_mid,Eigen::Vector3d &linear_angular_mid) {
    //
    ...
    // get deltas:
    Eigen::Vector3d angular_delta; 
    GetAngularDelta(1, 0, angular_delta,linear_angular_mid);
...
void ErrorStateKalmanFilter::SetProcessEquation(
    ...
    B_.block<3, 3>(INDEX_ERROR_GYRO, 6) = Eigen::Matrix3d::Identity();
    B_.block<3, 3>(INDEX_ERROR_ACCEL, 9) = Eigen::Matrix3d::Identity();
}
...
void ErrorStateKalmanFilter::UpdateErrorEstimation(
    ...
    UpdateProcessEquation(linear_acc_mid,linear_angular_mid);
...
void ErrorStateKalmanFilter::CorrectErrorEstimationPose(
    ...
    Eigen::Matrix3d C_bb_obs = T_nb.block<3, 3>(0,0).transpose() * pose_.block<3, 3>(0, 0); // fix this
...
void ErrorStateKalmanFilter::EliminateError(void) {
    ...
    pose_.block<3, 3>(0, 0) *= C_bb; // fix this
